...
int opt_evict() {
	...
	for (i = 0; i < memsize; i++){
		...
		while (curr != NULL){
			...
			if (curr->vaddr == curr_vaddr){
				// if we found frame that is accessed later than the one 
				// found before
				...
				inf_time = 0;
				// break, because we want the next access, not the last
				break;
			}
...
void opt_ref(pgtbl_entry_t *p) {
	...
	phys_frame[PGTBL_INDEX(p->frame)] = root->vaddr;
...
void opt_init() {
	...
	while(fgets(buf, MAXLINE, tfp) != NULL) {
		if(buf[0] != '=') {
			...
			// it was access_mem in sim.c, but we don't care about the type of access
			next_vaddr_ref(vaddr); 
		} else {
			continue;
		}
	}
